...
from myGames.faster.qbboxgame import Bboxgame
...
def run_model(max_moves=1214494, nb_epoch=2000, batch_size=100, epsilon=[1, 0.1], epsilon_rate=0.95, \
		lr=0.2, act1='relu', act2='relu', hid1=100, hid2=100, error="mse", reg_param=0.01):
	...
	print "training"
	agent.train(game, batch_size=batch_size, nb_epoch=nb_epoch, epsilon=epsilon, epsilon_rate=epsilon_rate)
	#print "playing"
	#agent.play(game)
	t1 = time.time()
	...
	if type(epsilon)  in {tuple, list}:
		log("{:^12}|{:^12}|{:^12.3f}{:^6.3f}{:^6.2f}|{:^10.2f}|{:^20}|{:>3.0f}:{:>02.0f}:{:>02.0f} |{:^6.2f}".format(\
			nb_epoch, batch_size, epsilon[0], epsilon[1], epsilon_rate, lr, act1[4:] + '(' + hid1+')' + " + " + act2[4:] + '(' + hid1+')', hrs, mins, sec, reg_param))
	else:
		log("{:^12}|{:^12}|{:^12.3f}{:^6.3}{:^6}|{:^10.2f}|{:^20}|{:>3.0f}:{:>02.0f}:{:>02.0f} |{:^6.2f}".format(\
			nb_epoch, batch_size, epsilon, "", "", lr, act1[4:] + '(' + hid1+')' + " + " + act2[4:] + '(' + hid1+')', hrs, mins, sec, reg_param))
...
run_model(nb_epoch=200, batch_size=100, epsilon=[1, 0.1], epsilon_rate=0.95, lr=0.2, act1='sigmoid', act2='sigmoid', hidden_size=100, error="mse", reg_param=0.1)
log(id)
run_model(nb_epoch=500, batch_size=100, epsilon=[1, 0.1], epsilon_rate=0.95, lr=0.2, act1='sigmoid', act2='sigmoid', hidden_size=100, error="mse", reg_param=0.1)
log(id)
run_model(nb_epoch=1000, batch_size=100, epsilon=[1, 0.1], epsilon_rate=0.95, lr=0.2, act1='sigmoid', act2='sigmoid', hidden_size=100, error="mse", reg_param=0.1)
log(id)
log("\n"+ id + "-fini: all runs all done\n")'''


#this is a new job - to run
'''id = str(111) + 'fast?'
log("\n"+ id + "-init: 10,000 moves - testing if cythonised works...")
run_model(max_moves=10000, nb_epoch=100, batch_size=100, epsilon=[1, 0.1], epsilon_rate=0.95, lr=0.2, act1='sigmoid', act2='sigmoid', hidden_size=100, error="mse", reg_param=0.1)
log(id)
run_model(max_moves=10000, nb_epoch=100, batch_size=100, epsilon=[1, 0.1], epsilon_rate=0.95, lr=0.2, act1='sigmoid', act2='sigmoid', hidden_size=100, error="mse", reg_param=0.1)
log(id)
run_model(max_moves=10000, nb_epoch=100, batch_size=100, epsilon=[1, 0.1], epsilon_rate=0.95, lr=0.2, act1='sigmoid', act2='sigmoid', hidden_size=100, error="mse", reg_param=0.1)
...
label = 'fast'
log('quick test run - ' + label)
run_model(max_moves=1000, nb_epoch=100, batch_size=1, epsilon=0.2, epsilon_rate=0.95, lr=0.2, act1='sigmoid', act2='sigmoid', hidden_size=100, error="mse", reg_param=0.1)
log('quick test run done - ' + label)
run_model(max_moves=1000, nb_epoch=100, batch_size=1, epsilon=0.2, epsilon_rate=0.95, lr=0.2, act1='sigmoid', act2='sigmoid', hidden_size=100, error="mse", reg_param=0.1)
log('quick test run done - ' + label)
run_model(max_moves=1000, nb_epoch=100, batch_size=1, epsilon=0.2, epsilon_rate=0.95, lr=0.2, act1='sigmoid', act2='sigmoid', hidden_size=100, error="mse", reg_param=0.1)
log('quick test run done - ' + label)
